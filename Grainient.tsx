...
float noise(vec2 p){vec2 i=floor(p),f=fract(p),u=f*f*(3.0-2.0*f);float n=mix(mix(dot(-1.0+2.0*hash(i+vec2(0.0,0.0)),f),dot(-1.0+2.0*hash(i+vec2(1.0,0.0)),f-vec2(1.0,0.0)),u.x),mix(dot(-1.0+2.0*hash(i+vec2(0.0,1.0)),f-vec2(0.0,1.0)),dot(-1.0+2.0*hash(i+vec2(1.0,1.0)),f-vec2(1.0,1.0)),u.x),u.y);return 0.5+0.5*n;}
...
const ctxMap = new WeakMap<HTMLDivElement, { renderer: Renderer; program: Program; mesh: Mesh; canvas: HTMLCanvasElement }>();
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uColorBalance: { value: -0.15 },
        uWarpStrength: { value: 0.5 },
        uWarpFrequency: { value: 3.2 },
        uWarpSpeed: { value: 0.7 },
        uWarpAmplitude: { value: 70 },
        uBlendAngle: { value: -12 },
        uBlendSoftness: { value: 0.18 },
        uRotationAmount: { value: 220 },
